...
После 3‑й сессии пользователь подтвердил план и сейчас идёт реализация **Группы A** roadmap.

**Очередь реализации (порядок согласован):**

1. **Schema versioning + миграции** в `ProjectRepository`. Сейчас `try/catch` молча возвращает `[]` при ошибке десериализации — потеря всех проектов. Это **блокер**: без него любое расширение `HouseProject.toJson` (нам нужны новые поля для расчёта фундамента) ломает существующие проекты.
2. **Выделение `cc_engine`** — отдельный pure‑Dart pub‑пакет в `packages/cc_engine/`. Туда переезжает `composition_planner.dart` и все будущие расчёты. Цели: тесты, переиспользование на сервере, отделение бизнес‑правил от Flutter.
3. **Эталонный модуль — фундамент**, 5 шагов:
   1. **Расчёт нагрузок** на фундамент: снег (СП 20.13330.2016, табл. 10.1 + карта снеговых районов 1‑VIII), ветер (СП 20, разд. 11), постоянные (от состава дома: вес стен, перекрытий, кровли), полезные (СП 20, табл. 8.3). Нужен справочник по регионам (доработать `region_catalog.dart`: снеговой район, ветровой район, расчётная температура).
   2. **Подбор подошвы** ленточного/плитного/свайного фундамента по СП 22.13330.2016: ширина подошвы из условия `p ≤ R` (расчётное сопротивление грунта), глубина заложения от глубины промерзания + типа грунта. **Армирование** по СП 63.13330.2018: сечение арматуры по моменту, шаг хомутов, защитный слой.
   3. **Пояснительная записка** — отдельный PDF: «Расчёт фундамента». Разделы: исходные данные, нагрузки (с формулами и подстановкой), подбор сечения, армирование, выводы. Ссылки на пункты СП с гиперссылкой.
   4. **Узел фундамента** — отдельный лист в основном PDF + DXF с размерами, маркировкой арматуры, защитным слоем, гидроизоляцией.
   5. **Строка ВОР** — таблица: бетон В25 м³, арматура А500С по диаметрам кг, опалубка м², гидроизоляция битум м², щебень м³. Класс `Bom` в моделях, аккумулирование по элементам.

4. **После фундамента — следующие пункты Группы A:** размеры/оси на чертежах, штамп СПДС, форматы листов, теплотехника, базовая смета, аккаунты, тарифы, юридический минимум, адаптив.
