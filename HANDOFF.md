...
«**Калькулятор проектировщика**» (construction_calculator) — Flutter‑приложение для расчёта зданий и сооружений. Сейчас полностью реализован один тип — **частный дом (ИЖС)**. Главная страница — landing с выбором типа сооружения (частный дом / многоквартирный / коммерческий / металлоконструкции; неготовые типы помечены «скоро»). Режим выбора пользователя (Клиент/Проектировщик) **удалён** — приложение всегда работает в полном режиме «Проектировщик» (ручное редактирование состава, полноэкранный редактор плана этажа, экспорт PDF/DXF, расчёты).
...
├── pages/
│   ├── home_page.dart                     # / — landing: hero, share, выбор типа сооружения, мои проекты
│   ├── settings_page.dart                 # Настройки (только переключатель подсказок)
...
**Текущая версия: v7.** Реализованы шаги 1, 2 и 3 эталонного модуля «Фундамент». Главная страница переделана в landing с выбором типа сооружения; выбор режима (Клиент / Проектировщик) удалён, всегда работает «Проектировщик».
...
- ✅ **Landing-главная** (`lib/pages/home_page.dart`): брендинг «КОНСТРУКТОР» в AppBar, hero-заголовок, кнопки шеринга (WhatsApp, Telegram, ВКонтакте, Одноклассники), описание, инструкция из 6 шагов в карточках, секция «Что вы хотите спроектировать?» с карточками типов сооружения, секция «Мои проекты» (если есть).
- ✅ **Выбор режима удалён.** `ModeSelectionPage` удалён, импорт убран. `AppState._mode` дефолтится к `UserMode.designer`. Settings page больше не показывает переключатель режима. Тексты подсказок (`Hints.home`) переписаны.
- ✅ **Карточки типов сооружения**: частный дом — кликабелен, открывает `ProjectCreatePage(initialType: ConstructionType.privateHouse)`. Многоквартирный / Коммерческое здание / Коммерческое сооружение / Металлоконструкции — диалог «в разработке» с пометкой «скоро».
- ✅ **Открытие соцсетей в новой вкладке**: новый сервис `lib/services/link_opener.dart` (с conditional imports `link_opener_stub.dart` / `link_opener_web.dart`) использует `web.window.open(url, '_blank')`.
- ✅ **`ProjectCreatePage`** теперь принимает `initialType` (опционально) — стартовое значение радио-группы типов.
- ✅ Тесты обновлены: 3 widget теста (`КОНСТРУКТОР` в AppBar, «Что вы хотите спроектировать?», «Инструкция из 6 шагов», `AppState.mode == designer` по дефолту). Установлен `setSurfaceSize(1280×1600)` для широких тестов.
