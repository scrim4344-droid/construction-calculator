...
class _FloorPlanEditorPageState extends State<FloorPlanEditorPage> {
  ...
  static const double _minOpeningLen = 0.6; // м (СП 55.13330)
  ...
  Future<void> _save() async {
    ...
    // Иначе — пересчитываем по правилам СП 55.13330, СП 1.13130, СП 23-102.
  ...
  void _onPanUpdateRoom(Offset localPos, FloorPlanTransform t) {
    ...
    if (_dragMode == _DragMode.move) {
      var nx = start.x + dx;
      var ny = start.y + dy;
      // Не даём вылезти за пятно застройки.
      ...
      switch (_anchor!) {
        case _ResizeAnchor.topLeft:
          left = (start.x + dx).clamp(0.0, right - _minSide).toDouble();
          top = (start.y + dy).clamp(0.0, bottom - _minSide).toDouble();
          break;
  ...
  void _onPanUpdateOpening(Offset localPos, FloorPlanTransform t) {
    ...
    if (_dragMode == _DragMode.move) {
      if (start.side.isHorizontal) {
        final nx =
            (start.x + dx).clamp(0.0, _plan.width - start.length).toDouble();
        updated = PlanOpening(
          kind: start.kind,
          side: start.side,
          x: nx,
        ...
        final ny =
            (start.y + dy).clamp(0.0, _plan.height - start.length).toDouble();
        updated = PlanOpening(
          kind: start.kind,
          side: start.side,
          x: start.x,
