...
class _LoadsContent extends StatelessWidget {
  ...
  /// Подобрать постоянные нагрузки от перекрытий — сейчас по умолчанию
  /// деревянные (типичная замена в ИЖС). В будущем будет выбираться из
  /// состава.
  List<WeightComponent> _floorsFromBrief() =>
  ...
  List<WeightComponent> _roofFromBrief(RoofDesign roof) {
    ...
    if (m.contains('soft') || m.contains('бит')) {
  ...
  Widget build(BuildContext context) {
    ...
    final result = FoundationLoadsCalculator.compute(
      ...
      windRegion: wind,
      roofShape: roofShape,
      roofSlopeDegrees: project.roof.slopeAngle ?? 30,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _row(
              'Материал кровли',
              project.roof.roofingMaterial ?? 'металлочерепица (по умолчанию)',
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'Дальше: подбор подошвы по СП 22.13330 и армирование по '
              'СП 63.13330. Появится в следующей итерации.',
